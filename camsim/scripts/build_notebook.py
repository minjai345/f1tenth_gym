...
md("# camsim 실습 노트북 — 카메라 기반 waypoint 모델: 시뮬 검증",
   ...
   "| 장 | 내용 | 바꿔 볼 값 |",
   "|---|---|---|",
   "| 1 | 카메라와 트랙 | 카메라 높이·pitch·화각, 테이프 배치(벽 추종/일정 폭), 색 |",
   "| 2 | 정답(GT)과 데이터셋 (BEV) | 정답 경로 기준(center/racing), pose 샘플링 범위, waypoint 거리, 장 수 |",
...
md("### 파라미터",
   ...
   "| 카메라, 테이프 배치(`follow_walls`, `wall_margin_m`, `track_width_m`), `waypoints.line`, `ahead_m`, `sampling.*` | 이 셀 + `REGENERATE = True` → 2장 → 3장 → 4장 |",
...
code('cfg = config.load()',
     ...
     'cfg.lane.follow_walls  = True     # True: 맵의 벽 안쪽을 따라 테이프를 놓는다 (폭이 구간마다 다름)',
     'cfg.lane.wall_margin_m = 0.25     # 벽에서 얼마나 안쪽에 놓을지 (follow_walls=True 일 때)',
     'cfg.lane.track_width_m = 0.8      # follow_walls=False 일 때의 좌우 테이프 간격 (실차 테이프 트랙 방식)',
     ...
     '# 증강은 기본 off (plain 모델). 아래 값은 3장 "sim-to-real" 과제에서 쓸 예시 증강의 세기다.',
     'cfg.augment.pitch_jitter_deg = 2.0    # 가감속 pitch 변화로 BEV 가 휘는 정도 (도)',
     'cfg.augment.ipm_blur_max_px = 9       # 먼 곳일수록 커지는 블러 (IPM 해상도 저하)',
...
md("### 트랙 어디인가",
   ...
   "`follow_walls = True` 면 테이프를 맵의 벽 안쪽 `wall_margin_m` 지점에 놓아 트랙 모양이 맵과 같아진다 (폭이 구간마다 다름).",
   "`False` 면 중심선에서 `track_width_m/2` 로 일정하게 놓는다 (실습실 테이프 트랙과 같은 방식).")
...
md("## 2. 정답(GT)과 데이터셋",
   ...
   "- `center` — 좌우 테이프의 중간선. 실차에서 HSV+IPM 으로 자동 라벨링할 때와 같은 기준이라 실차로 그대로 넘어간다.",
   "- `racing` — CSV 에 들어 있는 최적 레이싱 라인. 코너 안쪽을 파고들어 랩타임이 짧지만, 실차 라벨을 같은 기준으로 만들려면 트랙마다 따로 최적화해야 한다.",
   "",
   "두 기준으로 각각 학습해 4장에서 **랩타임과 횡오차·완주 안정성을 비교**해 보라 (`REGENERATE = True` 로 데이터부터 다시 만들어야 한다).",
...
md("### sim-to-real 갭: plain 모델은 실차에서 무너진다",
   ...
   "",
   "| 실차에서 생기는 일 | BEV에 나타나는 모양 | 함수 (OpenCV 기반) |",
   "|---|---|---|",
   "| 가감속 시 차체 pitch 변화 (서스펜션) | IPM 평면 가정이 깨져 먼 곳이 휨 | `jitter_bev` (기하학적으로 정확) |",
   "| IPM 원거리 해상도 저하 | 먼 테이프가 뭉개짐 (1장 오른쪽 그림) | `ipm_blur` |",
   "| 테이프 마모·벗겨짐, 다른 차의 가림 | 테이프 일부가 사라짐 | `erase_patches`, `dropout_quads` |",
...
code('degradations = {',
     ...
     'display(tbl)',
     '',
     '# 각 열화가 BEV 를 실제로 어떻게 바꾸는지 눈으로 확인',
...
md("### 과제: sim-to-real 증강 설계",
   ...
   "",
   "`camsim/augment.py` 함수들을 조합하거나 OpenCV 로 직접 써도 된다. 지나친 증강은 clean 성능을 깎으니 표로 확인할 것.",
   "주의: `jitter_bev` 를 뺀 나머지는 실제 카메라 물리를 근사한 것이다. 실차 영상을 찍고 나면 실제로 무엇이 깨지는지 보고 다시 설계하는 것이 맞다.")
code('def my_augment(bev, rng):',
     '    """학생이 채우는 부분. 주석을 풀거나 직접 OpenCV 로 새로 써도 된다. 세기는 파라미터 셀의 cfg.augment.*"""',
     '    out = bev',
...
code('env = closed_loop.make_env(cfg)',
     ...
     'print(f"기준 경로 {cfg.waypoints.line} (길이 {trk.length:.1f} m), 제어 주기 {cfg.closed_loop.control_hz} Hz, 속도 {cfg.closed_loop.speed_mps} m/s, lookahead {cfg.closed_loop.lookahead_m} m")',
