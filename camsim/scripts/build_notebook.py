...
code('ds_train = dataset.DiskDataset(DATA_DIR, cfg, "train", val_frac=0.1)',
     ...
     '    plt.plot(st, [h["loss"] for h in history], label="train")',
     '    if "val_loss" in history[-1]:',
     '        plt.plot(st, [h["val_loss"] for h in history], label="val")',
     ...
     '                        dataset=ds_train, val_dataset=ds_val, val_batches=8, callback=live_plot)',
     'print("model.pt 저장")')
