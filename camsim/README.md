...
`center` = 좌우 테이프의 중간선 (실차 HSV+IPM 자동 라벨링과 같은 기준, 기본값).
`racing` = `examples/example_waypoints.csv` 에 들어 있는 최적 레이싱 라인 (코너 안쪽을 파고들어 더 짧다: 156 m vs 163 m).
어느 쪽이든 테이프(모델이 보는 것)는 같고 정답만 달라진다. 두 기준으로 학습해 랩타임·안정성을 비교하는 것이 노트북 2장의 실습이다.
`Track.left_m` / `right_m` 이 그 기준선에서 좌우 테이프까지 거리이며, pose 샘플링과 실격 판정이 이 값을 쓴다.
